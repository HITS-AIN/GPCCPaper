...
function createplots_joint_reorder()
    ...
    axtop   = Axis(GL[1, 1], title  = L"\textrm{marginal delay between 7700}\AA \textrm{ and 9100}\AA",  xlabel = "delays (days)", ylabel = L"\pi_i", xgridvisible = false, ygridvisible = false)
    xlims!(axtop, 0,10)
    axtop.xticks = 0:2:10
   
    axmain  = Axis(GL[2, 1], title = "joint posterior", xlabel = L"\textrm{delay between 7700}\AA \textrm{ and 9100}\AA", ylabel = L"\textrm{delay between 5100}\AA\textrm{ and 9100}\AA", xgridvisible = false, ygridvisible = false)
    xlims!(axmain, 0,10)
    ylims!(axmain, 0,10)
    axmain.yticks = 0:2:10
    axmain.xticks = 0:2:10
   
    axright = Axis(GL[2, 2], title  = L"\textrm{marginal delay between 5100}\AA\textrm{ and 9100}\AA", ylabel = "delays (days)", xlabel = L"\pi_i", xgridvisible = false, ygridvisible = false)
    ylims!(axright, 0,10)
